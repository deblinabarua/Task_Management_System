...
with tab2:
    ...
    with st.form("Add Task"):
        project = st.selectbox("Project", get_projects, format_func = lambda x:x["title"])
        ...
        if submit:
            add = requests.post(f"{API_URL}/create_task", json = {"projectid": project["projectid"], "title": title, "description": description, "position": position, "parent_task": int(parent) if parent else None, "created_by": curr_user, "members": [curr_user]})
